...
def create_invoice(
    ...
    Args:
        ...
        file_path (str, optional): The path to save the PDF file. Required if return_bytes is False.
        ...
        return_bytes (bool): If True, returns PDF as bytes instead of saving to file.

    Returns:
        bytes or None: PDF bytes if return_bytes is True, otherwise None.
    """
    if headers is None:
        headers = {
    ...
    # Create canvas - either to file or to buffer
    if return_bytes:
        buffer = io.BytesIO()
        c = canvas.Canvas(buffer, pagesize=letter)
    else:
        if not file_path:
            raise ValueError("file_path is required when return_bytes is False")
